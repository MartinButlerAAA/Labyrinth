...
{ 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},   // U+0023 (#) changed to a solid block fo Labyrinth
...
{ 0x00, 0x08, 0x0C, 0x7E, 0x7E, 0x0C, 0x08, 0x00},   // U+003C (<) changed to a left arrow for Labyrinth
{ 0x00, 0x00, 0x3F, 0x00, 0x00, 0x3F, 0x00, 0x00},   // U+003D (=)
{ 0x00, 0x10, 0x30, 0x7E, 0x7E, 0x30, 0x10, 0x00},   // U+003E (>) changed to a right arrow for Labyrinth
...
{ 0x00, 0x18, 0x3C, 0x7E, 0x18, 0x18, 0x18, 0x00},   // U+005E (^) changed to an up arrow for Labyrinth
{ 0x00, 0x18, 0x18, 0x18, 0x7E, 0x3C, 0x18, 0x00},   // U+005F (_) changed to a down arrow for Labyrinth
...
static float mySqrt(float square)
{
	...
	for (int a = 6; a > 0; a--) 
	{
		...
		// Use the digit before the one that pused the total above the square. 
...
// Draw a line that is limited to the specified display area.
bool drawLineLtd(float x1, float y1, float x2, float y2, unsigned int colour)
{
	float x1l, x2l, y1l, y2l;	// local working variables.

	// Copy the pass parameters to local variables.
	x1l = x1;
	x2l = x2;
	y1l = y1;
	y2l = y2;

	// Limit the local veriables to the display area.
	if (x1l < 0.0)      { x1l = 0.0; }
	if (x2l < 0.0)      { x2l = 0.0; }
	if (y1l < 0.0)      { y1l = 0.0; }
	if (y2l < 0.0)      { y2l = 0.0; }
	if (x1l > XDISPMAX) { x1l = XDISPMAX; }
	if (x2l > XDISPMAX) { x2l = XDISPMAX; }
	if (y1l > YDISPMAX) { y1l = YDISPMAX; }
	if (y2l > YDISPMAX) { y2l = YDISPMAX; }

	// Add the offsets for the start of the display area
	x1l = x1l + XOFFSET;
	x2l = x2l + XOFFSET;
	y1l = y1l + YOFFSET;
	y2l = y2l + YOFFSET;

	// Call the standard drawLine function with the limited x and y values.
	return (drawLine(x1l, y1l, x2l, y2l, colour));
}
